...
当前进度：运行中心、歌单、工作流、目标库和设置已接入 Vue；真实动作执行、全局日志、F8 点位采集、点位预览、窗口拖拽选区、后台截图、模板/遮罩编辑、测试识别及图像动作确定性恢复已由 Python sidecar 提供。运行中心现可生成完整运行计划，提前检查变量、目标引用与时长，并通过加速演练验证队列调度而不发送输入。前端使用通用 `RunItem` 适配器读取现有歌单数据，后续可接入非音乐任务源而无需破坏存量 JSON。下一轮聚焦动作后状态验证、条件分支与通用任务集模型；剧组站页面仍待纵向迁移。
