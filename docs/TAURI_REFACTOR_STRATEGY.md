...
- 现代化设计,深色主题可以保留，后续可以更改主题.但避免大面积毛玻璃、装饰渐变和过度卡片化。
...
- 不在界面中堆放“如何使用本界面”的说明文字。但可以有帮助按钮查询
...
当前进度：阶段 4 的主要页面均已接入 Vue。运行中心、队列、工作流、目标库和设置通过 Python sidecar 完成真实执行、日志、F8 点位采集、点位预览、窗口选区、后台截图、模板/遮罩编辑、测试识别及确定性失败恢复。剧组站已迁移参数读取与保存、一次性 UAC 网络监听、参数验证、分类搜索、时长补全、受限封面代理、结果排序和加入队列；真实 `skey`、私人队列和模板仍只保存在本地忽略文件中。

阶段 4 的代码迁移已经覆盖高级诊断，尚余真实 Windows/游戏环境中的完整功能对照验收。验收通过后进入阶段 5，优先打包 Python/OpenCV/WinDivert sidecar 并验证 NSIS 安装包；条件分支、通用任务集和新的动作类型在迁移完成后再开始。
