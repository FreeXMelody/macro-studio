...
当前进度：运行中心、歌单、工作流、目标库和设置已接入 Vue；真实动作执行、全局日志、F8 点位采集、点位预览、窗口拖拽选区、后台截图、模板/遮罩编辑及测试识别已由 Python sidecar 提供。下一轮聚焦图像动作的确定性重试、失败恢复、点击后界面验证与完整歌单可靠性验收；剧组站页面仍待纵向迁移。
